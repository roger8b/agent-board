...
const PREFIX = "PROJ";
let counter = 1; // PROJ-001 == board id
const nextId = () => `${PREFIX}-${String(++counter).padStart(3, "0")}`;
...
async function main() {
  ...
  const boardId = `${PREFIX}-001`;
  await prisma.board.create({
    data: { id: boardId, name: "Quadro Kanban", prefix: PREFIX, counter: 1 },
  });
    ...
    for (let j = 0; j < list.length; j++) {
      ...
      if (c.slug === "backlog" && j === 0) {
        await prisma.subTask.create({
          data: { id: nextId(), taskId, title: "Revisar logs de ontem", description: "Verificar ontem à noite", status: "done", done: true, order: 0 },
        });
        await prisma.subTask.create({
          data: { id: nextId(), taskId, title: "Identificar padrão de erros", status: "", done: false, order: 1 },
        });
  ...
  // persist the counter so future creations continue the sequence
  await prisma.board.update({ where: { id: boardId }, data: { counter } });
  console.log(`Seed concluído. IDs ${PREFIX}-001..${PREFIX}-${String(counter).padStart(3, "0")}`);
}
